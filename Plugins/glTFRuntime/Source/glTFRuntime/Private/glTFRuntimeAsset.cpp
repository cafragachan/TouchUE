...
bool UglTFRuntimeAsset::LoadFromFilename(const FString& Filename, const FglTFRuntimeConfig& LoaderConfig)
{
	...
	if (Parser)
	{
		...
		////test
		//Delegate.BindUFunction(this, GET_FUNCTION_NAME_CHECKED(UglTFRuntimeAsset, OnPrimitiveLoadedProxy));
		//Parser->OnPrimitiveLoaded.Add(Delegate);
	}
...
void UglTFRuntimeAsset::OnStaticMeshCreatedProxy(UStaticMesh* StaticMesh)
{
	if (OnStaticMeshCreated.IsBound())
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("testing Delegate mesh"));
...
//void UglTFRuntimeAsset::OnPrimitiveLoadedProxy(TArray<UMaterialInterface*> MaterialVariants)
//{
//	if (OnPrimitiveLoaded.IsBound())
//	{
//		OnPrimitiveLoaded.Broadcast(MaterialVariants);
//		UE_LOG(LogTemp, Warning, TEXT("testing Delegate material variants 1"));
//	}
//}
...
TArray<UMaterialInterface*> UglTFRuntimeAsset::GetMaterialVariantExtensions()
{
	return Parser->GetMaterialVariants();
}