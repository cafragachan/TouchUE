...
void AglTFRuntimeAssetActor::ProcessNode(USceneComponent* NodeParentComponent, FglTFRuntimeNode& Node)
{
	...
	//UE_LOG(LogTemp, Warning, TEXT("num of materials in asset : %i"), Asset->GetMaterialVariantExtensions().Num());

	if (Asset->GetMaterialVariantExtensions().Num() != 0)
	{
		MaterialVariants = Asset->GetMaterialVariantExtensions();
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("No Material Variants Found"));
