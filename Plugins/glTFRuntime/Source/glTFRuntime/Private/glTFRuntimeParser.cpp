...
bool FglTFRuntimeParser::LoadPrimitive(TSharedRef<FJsonObject> JsonPrimitiveObject, FglTFRuntimePrimitive& Primitive, const FglTFRuntimeMaterialsConfig& MaterialsConfig)
{
	...
	const TSharedPtr<FJsonObject>* JsonVarExtensions;

	if (!JsonPrimitiveObject->TryGetObjectField("extensions", JsonVarExtensions))
	{
		UE_LOG(LogTemp, Warning, TEXT("no extensions found in Primitive"));
		return false;
	}

	// KHR_materials_variants
	const TSharedPtr<FJsonObject>* JsonMaterialVariants;
	if ((*JsonVarExtensions)->TryGetObjectField("KHR_materials_variants", JsonMaterialVariants))
	{

		TSharedPtr<FJsonValue> Field = (*JsonMaterialVariants)->TryGetField("mappings");

		if (!Field.IsValid())
		{
			UE_LOG(LogTemp, Warning, TEXT("no variants in field"));
			return false;
		}
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("no arrays in field"));
			return false;
		}
		...
		{
			TSharedPtr<FJsonObject> JsonMatObject = (*Array)[i]->AsObject();
			if (!JsonMatObject)
				continue;

			int32 VarMaterialIndex;

			TSharedPtr<FJsonValue> NameValue = JsonMatObject->TryGetField("material");
			if (NameValue)
			{
				VarMaterialIndex = (int32)NameValue.Get()->AsNumber();
				//UE_LOG(LogTemp, Warning, TEXT("Material %i name: %i"), i, index);
			}
			...
			TempMaterial = LoadMaterial(VarMaterialIndex, MaterialsConfig, Primitive.Colors.Num() > 0, Primitive.MaterialName);

			if (TempMaterial)
			{
				MaterialsVariantsArray.Add(TempMaterial);
				UE_LOG(LogTemp, Warning, TEXT("Material index: %i"), VarMaterialIndex);

			}
			else
			{
				UE_LOG(LogTemp, Warning, TEXT("material variant failure"));

			}
			
			/*if (OnPrimitiveLoaded.IsBound())
			{
				OnPrimitiveLoaded.Broadcast(MaterialVariants);
				UE_LOG(LogTemp, Warning, TEXT("testing Delegate material variants"));

			}*/

		}

		UE_LOG(LogTemp, Warning, TEXT("Material variants found: %i"), Array->Num());
	}
...
TArray< UMaterialInterface*> FglTFRuntimeParser::GetMaterialVariants()
{
	return MaterialsVariantsArray;
}
